...
def entropy(class_y):
    ...
    if (len(class_y) == 0) or (np.sum(class_y) == len(class_y)) or (np.sum(class_y) == 0):
        return 0
    else:
        p1 = np.sum(class_y)/len(class_y)
        p0 = 1-p1
...
def partition_classes(X, y, split_attribute, split_val):
    ...
    if isinstance(X[0][split_attribute], float) or isinstance(X[0][split_attribute], int):
        X_left = np.array([x for x in X if x[split_attribute] <= split_val])
        X_right = np.array([x for x in X if x[split_attribute] > split_val])
        y_left = np.array([y[i] for i in range(len(y)) if X[i][split_attribute] <= split_val])
        y_right = np.array([y[i] for i in range(len(y)) if X[i][split_attribute] > split_val])
    else:
        X_left = np.array([x for x in X if x[split_attribute] == split_val])
        X_right = np.array([x for x in X if x[split_attribute] != split_val])
        y_left = np.array([y[i] for i in range(len(y)) if X[i][split_attribute] == split_val])
